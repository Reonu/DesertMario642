...
enum BillboardIDs billboardList[BB_BILLBOARD_END] = {
    ...
    [BB_SIGN_JOOPII]             = (u32) BB_IMAGE_JOOPII,
...
struct DMAVideoProperties videoDMAProps[BB_VIDEO_COUNT] = {
    ...
    [BB_VIDEO_DACHI]        = {.addr = dachi_video_data,            .billboardId = BB_SIGN_DACHI,        .relativeLoopStart = 0, .frameTotal = ARRAY_COUNT(dachi_video_data),            .startFrame = 0, .framerate = 15, .sound = SOUND_BB_DACHI       },
...
Gfx *geo_billboard_image_scene(s32 callContext, struct GraphNode *node, UNUSED void *context) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        if (currentGraphNode->parameter == LAYER_OPAQUE) {
            ...
            gSPDisplayList(dlHead++, sign_normal_sign_idiot_mesh_layer_1_idiot_mario_begin);
            gDPSetTextureImage(dlHead++, G_IM_FMT_RGBA, G_IM_SIZ_32b_LOAD_BLOCK, 1, VIRTUAL_TO_PHYSICAL(rhdcPFPRGBA32));
            gSPDisplayList(dlHead++, sign_normal_sign_idiot_mesh_layer_1_idiot_mario_end);
            gSPEndDisplayList(dlHead);
        }
    }
