...
void mode_8_directions_camera(struct Camera *c) {
...
#endif
#ifndef DESERT_DEBUG
        if (gMarioState->action != ACT_UNPROCESSED && gMarioCurrentRoom != 2) {
            s8DirModeYawOffset++;
        }
#endif
    }
    ...
    if (gCurrLevelNum == LEVEL_DESERT_INTRO) {
        ...
        s8DirModeYawOffset += DEGREES(0.25f);
    }
...
struct CameraTrigger sCamDesert[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamDesertIntro[] = {
	NULL_TRIGGER
};
struct CameraTrigger sCamVegasEnding[] = {
	NULL_TRIGGER
};
